...
repsundown = np.tile(np.tile(sundown, (1, 1, 1, 1, 1)),
                     (7, 7, 1, 1, 1))(order=[2, 1, 0, 3, 4])
# where(condition, x, y) is x where condition is true, y otherwise
SW1 = MV.where(repsundown == 0, 0, SW0)
# where(condition, x, y) is x where condition is true, y otherwise
SW_cld_fbk = MV.where(repsundown.mask, 0, SW1)
...
print('avg LW cloud feedback = {}'.format(str(avgLW_cld_fbk)))
...
print('avg SW cloud feedback = {}'.format(str(avgSW_cld_fbk)))
...
pl.subplot(221)
data = cdutil.averager(MV.average(avgclisccp1_grd, axis=0),
                       axis='xy', weights='weighted').transpose()
...
pl.subplot(222)
data = cdutil.averager(MV.average(anomclisccp, axis=0),
                       axis='xy', weights='weighted').transpose()
...
pl.subplot(223)
data = cdutil.averager(MV.average(LW_cld_fbk, axis=0),
                       axis='xy', weights='weighted').transpose()
pl.pcolor(data, shading='flat', cmap='RdBu_r', vmin=-0.75, vmax=0.75)
pl.xticks(np.arange(8), tau)
pl.yticks(np.arange(8), ctp)
...
pl.subplot(224)
data = cdutil.averager(MV.average(SW_cld_fbk, axis=0),
                       axis='xy', weights='weighted').transpose()
pl.pcolor(data, shading='flat', cmap='RdBu_r', vmin=-0.75, vmax=0.75)
pl.xticks(np.arange(8), tau)
pl.yticks(np.arange(8), ctp)
...
cb = pl.colorbar()
cb.set_label('W/m$^2$/K')
pl.savefig('/home/shofer/Documents/repos/cloud-radiative-kernels/cloud_kernel_example_hists.png',
           bbox_inches='tight')  # SH
...
# this is the critical line for making sure the colors vary linearly even if the desired color boundaries are at varied intervals
norm = mpl.colors.BoundaryNorm(bounds2, cmap.N)
pl.subplot(2, 1, 1)
m = Basemap(projection='robin', lon_0=210)  # 'hammer',lon_0=180)
m.drawmapboundary(fill_color='0.3')
im1 = m.contourf(LON, LAT, sumLW, bounds, shading='flat',
                 cmap=cmap, norm=norm, latlon=True, extend='both')
m.drawcoastlines(linewidth=1.5)
cb = pl.colorbar(im1, orientation='vertical', drawedges=True, ticks=bounds)
pl.title(
    'LW Cloud Feedback [' + str(np.round(avgLW_cld_fbk, 3)) + ']', fontsize=14)
cb.set_label('W/m$^2$/K')

pl.subplot(2, 1, 2)
m = Basemap(projection='robin', lon_0=210)  # 'hammer',lon_0=180)
m.drawmapboundary(fill_color='0.3')
im1 = m.contourf(LON, LAT, sumSW, bounds, shading='flat',
                 cmap=cmap, norm=norm, latlon=True, extend='both')
m.drawcoastlines(linewidth=1.5)
cb = pl.colorbar(im1, orientation='vertical', drawedges=True, ticks=bounds)
pl.title(
    'SW Cloud Feedback [' + str(np.round(avgSW_cld_fbk, 3)) + ']', fontsize=14)
cb.set_label('W/m$^2$/K')
pl.savefig('/home/shofer/Documents/repos/cloud-radiative-kernels/cloud_kernel_example_maps.png',
           bbox_inches='tight')  # SH
